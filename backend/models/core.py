...
class BlobObject(Base):
    ...
    __table_args__ = (
        UniqueConstraint('tenant_id', 'source_type', 'business_date', 'content_hash', name='uq_blob_dedup'),
    )
...
class IngestionRun(Base):
    ...
    tenant_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('tenants.id'), nullable=False)
    business_date: Mapped[date] = mapped_column(Date, nullable=False)
    status: Mapped[RunStatus] = mapped_column(SAEnum(RunStatus), nullable=False, default=RunStatus.RECEIVED)
    bank_blob_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey('blob_objects.id'))
    remittance_blob_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey('blob_objects.id'))
