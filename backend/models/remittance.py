...
class RemittanceAdviceHeader(Base):
    ...
    blob_object_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("blob_objects.id"), unique=True, nullable=False
    )
    buyer_reference: Mapped[str | None] = mapped_column(String(80))
    ...
    source_blob: Mapped[BlobObject] = relationship(
        back_populates="remittance_advice_header"
    )
    lines: Mapped[list[RemittanceAdviceLine]] = relationship(
        back_populates="advice_header"
    )
...
class RemittanceAdviceLine(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "advice_header_id", "line_number", name="uq_remittance_line_number"
        ),
    )
    ...
    advice_header_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("remittance_advice_headers.id"), nullable=False
    )
    ...
    matches: Mapped[list[ReconciliationMatch]] = relationship(
        back_populates="remittance_advice_line"
    )
