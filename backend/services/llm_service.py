...
def _heuristic_extract(text: str) -> RemittanceExtractionResult:
    ...
    amount_re = re.compile(
        r"([0-9]{1,3}(?:[\.,][0-9]{3})*[\.,][0-9]{2})\s*(EUR|€)?", re.IGNORECASE
    )
    invoice_re = re.compile(
        r"(?:RECHNUNG|INVOICE|RNR|RG)\s*[:#-]?\s*([A-Z0-9\-/]+)", re.IGNORECASE
    )

    for idx, line in enumerate(
        [raw_line.strip() for raw_line in text.splitlines() if raw_line.strip()],
        start=1,
    ):
    ...
    for pattern in ("%Y-%m-%d", "%d.%m.%Y", "%d/%m/%Y", "%d-%m-%Y"):
        try:
            return (
                ...
                if pattern == "%Y-%m-%d"
                else datetime.strptime(text, pattern).date().isoformat()
            )
        except Exception:
            continue
    return text
...
def _normalize_payload(payload: dict) -> dict:
    ...
    # Handle fields renamed from older LLM responses for backward compatibility
    if "advice_number" in normalized and "buyer_reference" not in normalized:
        normalized["buyer_reference"] = normalized.pop("advice_number")
    # Handle field renamed from older LLM responses for backward compatibility
    if "payer_name" in normalized and "buyer_name" not in normalized:
        normalized["buyer_name"] = normalized.pop("payer_name")
    normalized["advice_date"] = _parse_dateish(normalized.get("advice_date"))
    normalized["total_paid_amount"] = _parse_decimalish(
        normalized.get("total_paid_amount")
    )
    normalized["parsing_confidence"] = _parse_decimalish(
        normalized.get("parsing_confidence")
    )
    ...
    for idx, raw_line in enumerate(raw_lines, start=1):
        ...
        item["currency"] = (
            item.get("currency").strip()[:3].upper()
            if isinstance(item.get("currency"), str) and item.get("currency").strip()
            else None
        )
        # Handle field renamed from older LLM responses
        if "customer_reference" in item and "buyer_reference" not in item:
            item["buyer_reference"] = item.pop("customer_reference")
        # Use assignment_text for German-format amount parsing
        assignment_text = item.get("assignment_text") or ""
        if isinstance(assignment_text, str):
            amounts = _extract_german_amounts(assignment_text)
            if len(amounts) >= 3:
...
def _extract_via_gemini_pdf_bytes(
    pdf_bytes: bytes,
) -> tuple[RemittanceExtractionResult, dict]:
...
def extract_remittance_with_llm(
    raw_text: str, pdf_bytes: bytes | None = None
) -> tuple[RemittanceExtractionResult, dict, str, str]:
    ...
    if not raw_text.strip() and pdf_bytes:
        try:
            ...
            return (
                result,
                {"mode": "llm_pdf_error", "error": str(exc)},
                "heuristic",
