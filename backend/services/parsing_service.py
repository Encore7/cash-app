...
BANK_COLS = {
    ...
    "IBAN": "iban",
    "Konto": "account_label",
    "Kontonummer": "buyer_account_number",
        ...
        rows.append(
            {
                ...
                "bank_reference": (
                    str(row_data.get("bank_reference"))
                    if row_data.get("bank_reference")
                    else None
                ),
                "buyer_reference": (
                    str(row_data.get("buyer_reference"))
                    if row_data.get("buyer_reference")
                    else None
                ),
                "buyer_account_number": (
                    str(row_data.get("buyer_account_number"))
                    if row_data.get("buyer_account_number")
                    else None
                ),
...
def validate_remittance_result(
...
def parse_remittance(
    content: bytes,
) -> tuple[RemittanceValidationReport, dict, str, str]:
    raw_text = extract_pdf_text(content)
    extracted, raw_llm_output, method, llm_model = extract_remittance_with_llm(
        raw_text, pdf_bytes=content
    )
